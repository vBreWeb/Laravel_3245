...
<section class="max-w-7xl mx-auto px-6 py-20 flex flex-col md:flex-row items-center gap-12">
    ...
    <section id="categories" class="max-w-7xl mx-auto px-6 py-20">
        <div class="mb-10">
            <h2 class="text-3xl font-extrabold mb-3">Kategori Event di AmikomEventHub</h2>
            <p class="text-slate-500">Platform ini mendukung berbagai kategori acara mulai dari seminar, workshop, kompetisi, hingga hiburan.</p>
        </div>
        <div class="flex flex-wrap gap-3">
            @forelse($categories as $category)
            <span class="px-4 py-2 bg-indigo-100 text-indigo-700 rounded-full font-semibold uppercase text-xs tracking-wider">{{ $category->name }}</span>
            @empty
            <span class="px-4 py-2 bg-slate-100 text-slate-500 rounded-full text-sm">Belum ada kategori tersedia.</span>
            @endforelse
        </div>
    </section>
    ...
    <section id="partners" class="max-w-7xl mx-auto px-6 py-20">
        <div class="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-4">
            <div>
                <h2 class="text-3xl font-extrabold mb-3">Partner Kerja Sama</h2>
                <p class="text-slate-500">Partner terpercaya yang mendukung kelancaran event di AmikomEventHub.</p>
            </div>
            <a href="{{ route('admin.partners.index') }}" class="px-6 py-3 bg-indigo-600 text-white rounded-2xl font-bold hover:bg-indigo-700 transition">Lihat Semua Partner</a>
        </div>
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-6">
            @forelse($partners as $partner)
            <div class="bg-white rounded-3xl border border-slate-100 shadow-sm p-6 flex flex-col items-center text-center">
                @if($partner->logo_url)
                <img src="{{ $partner->logo_url }}" alt="Logo {{ $partner->name }}" class="w-24 h-24 object-contain mb-4">
                @else
                <div class="w-24 h-24 bg-slate-100 rounded-3xl mb-4 flex items-center justify-center text-slate-400">Logo</div>
                @endif
                <h3 class="font-bold text-slate-900">{{ $partner->name }}</h3>
                <p class="text-xs text-slate-500 mt-2">Bergabung sejak {{ $partner->created_at->format('M Y') }}</p>
            </div>
            @empty
            <div class="col-span-full bg-slate-50 rounded-3xl border border-slate-200 p-10 text-center text-slate-500">
